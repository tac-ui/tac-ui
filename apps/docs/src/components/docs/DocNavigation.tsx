...
function useAdjacentPages(groups: NavGroup[]) {
...
export function DocNavigation({ navGroups = defaultNavGroups }: { navGroups?: NavGroup[] }) {
  const { prev, next } = useAdjacentPages(navGroups);
    ...
    <nav className="grid grid-cols-2 gap-3 pt-8 mt-4 border-t border-solid border-[var(--border)]">
      {prev ? (
        ...
        >
          <ChevronLeft
            size={16}
            className="text-[var(--muted-foreground)] shrink-0 transition-transform group-hover:-translate-x-0.5"
          />
          <div className="flex flex-col min-w-0">
            <span className="text-[11px] text-[var(--muted-foreground)]">{t.ui.prevPage ?? 'Previous'}</span>
      ...
      {next ? (
        ...
        >
          <div className="flex flex-col items-end min-w-0">
            <span className="text-[11px] text-[var(--muted-foreground)]">{t.ui.nextPage ?? 'Next'}</span>
            <span className="text-[13px] font-medium text-[var(--foreground)] truncate">
