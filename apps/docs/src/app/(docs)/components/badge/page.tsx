  ...
  return (
    <DocPage>
      <div>
        ...
        <DocDescription>{pt?.description ?? 'A small status label used to display counts, states, or categories.'}</DocDescription>
      </div>

      <DocSection title={pt?.sections?.['variants']?.title ?? 'Variants'}>
        <DocText>{pt?.sections?.['variants']?.texts?.[0] ?? 'Nine semantic variants cover the full range of status communication from neutral to critical.'}</DocText>
        <Showcase
...
<Badge variant="info">Info</Badge>`}
      ...
      <DocSection title={pt?.sections?.['glass']?.title ?? 'Glass'}>
        <DocText>{pt?.sections?.['glass']?.texts?.[0] ?? 'The glass variant uses a frosted-glass backdrop filter with a subtle border, adapting naturally to both light and dark themes.'}</DocText>
        <Showcase
...
<Badge variant="glass">Pro</Badge>`}
      ...
      <DocSection title={pt?.sections?.['interactive']?.title ?? 'Interactive'}>
        <DocText>{pt?.sections?.['interactive']?.texts?.[0] ?? 'Set interactive or provide an onClick to enable spring hover and tap animations. Useful for filterable tag chips.'}</DocText>
        <Showcase
...
<Badge variant="success" onClick={() => alert('clicked')}>Status</Badge>`}
      ...
      <DocSection title={pt?.sections?.['count']?.title ?? 'Count'}>
        <DocText>{pt?.sections?.['count']?.texts?.[0] ?? 'The count prop animates numeric value changes with a popLayout exit transition. Use it for notification counts or unread indicators.'}</DocText>
        <Showcase
        ...
        <PropsTable
          data={[
            ...
            },
            {
              ...
              type: 'number',
              default: '-',
              description: pt?.props?.['count'] ?? 'Numeric value to display. When provided, changes animate with an exit transition via AnimatePresence.',
            },
            {
              name: 'interactive',
              type: 'boolean',
              default: 'false',
              description: pt?.props?.['interactive'] ?? 'Enables spring hover and tap animations. Automatically enabled when onClick is provided.',
            },
            {
              name: 'onClick',
            ...
            },
            {
              ...
              default: '-',
              description: pt?.props?.['children'] ?? 'Content displayed inside the badge. Ignored when count is provided.',
            },
            {
              name: 'className',
              type: 'string',
