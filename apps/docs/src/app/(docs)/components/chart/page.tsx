  ...
  return (
    <DocPage>
      <div>
        ...
        <DocDescription>{pt?.description ?? 'Responsive chart components built on Chart.js with Tac UI design tokens. Minimal grid lines, restrained monochrome palette, and clean typography.'}</DocDescription>
      </div>

      <DocSection title={pt?.sections?.['bar-chart']?.title ?? 'Bar Chart'}>
        <DocText>{pt?.sections?.['bar-chart']?.texts?.[0] ?? 'Vertical bars with per-bar color via the chart token palette. Grid lines are near-invisible for a clean canvas.'}</DocText>
        <Showcase
          code={`<BarChart
  data={[
    { label: 'Jan', value: 42 },
...
/>`}
      ...
      <DocSection title={pt?.sections?.['line-chart']?.title ?? 'Line Chart'}>
        <DocText>{pt?.sections?.['line-chart']?.texts?.[0] ?? 'Smooth line with optional area fill and data points. Uses a single color from the chart palette.'}</DocText>
        <Showcase
          code={`<LineChart
  data={[
    { label: 'Mon', value: 20 },
...
/>`}
      ...
      <DocSection title={pt?.sections?.['pie-chart']?.title ?? 'Pie Chart'}>
        <DocText>{pt?.sections?.['pie-chart']?.texts?.[0] ?? 'Proportional slices with legend. Borders blend into the background for clean separation.'}</DocText>
        <Showcase
          code={`<PieChart
  data={[
    { label: 'Desktop', value: 54 },
...
/>`}
      ...
      <DocSection title={pt?.sections?.['donut-chart']?.title ?? 'Donut Chart'}>
        <DocText>{pt?.sections?.['donut-chart']?.texts?.[0] ?? 'Donut variant with a refined ring width. Hover for percentage tooltips.'}</DocText>
        <Showcase
          code={`<DonutChart
  data={[
    { label: 'Completed', value: 48 },
        ...
        <PropsTable
          data={[
            ...
            },
            {
              ...
              type: '"pie" | "donut"',
              default: '"pie"',
              description: pt?.props?.['pie-variant'] ?? 'Whether to render as a pie or a donut. DonutChart defaults to "donut".',
            },
            {
              name: 'strokeWidth',
              type: 'number',
            ...
            },
            {
              ...
              type: 'boolean',
              default: 'false',
              description: pt?.props?.['pie-showValues'] ?? 'Whether to show percentage values (donut: center total; pie: in slice).',
            },
          ]}
        />
      </DocSection>
