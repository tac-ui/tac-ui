  ...
  return (
    <DocPage>
      <div>
        ...
        <DocDescription>{pt?.description ?? 'A loading placeholder that mimics content layout while data is being fetched.'}</DocDescription>
      </div>
      ...
      <DocSection title={pt?.sections?.['rectangular']?.title ?? 'Rectangular'}>
        <DocText>{pt?.sections?.['rectangular']?.texts?.[0] ?? 'Rectangle placeholder for images, cards, or blocks.'}</DocText>
        <Showcase
          className="flex-col items-stretch"
      ...
      <DocSection title={pt?.sections?.['circular']?.title ?? 'Circular'}>
        <DocText>{pt?.sections?.['circular']?.texts?.[0] ?? 'Circle placeholder typically used for avatars or icons.'}</DocText>
        <Showcase
...
<Skeleton variant="circular" width={72} height={72} />`}
      ...
      <DocSection title={pt?.sections?.['animation']?.title ?? 'Animation'}>
        <DocText>{pt?.sections?.['animation']?.texts?.[0] ?? 'Choose between a sweeping shimmer highlight or a subtle pulse fade.'}</DocText>
        <Showcase
          className="flex-col items-stretch"
...
<Skeleton animation="pulse" width="100%" height={48} />`}
      ...
      <DocSection title={pt?.sections?.['card-composition']?.title ?? 'Card Composition'}>
        <DocText>{pt?.sections?.['card-composition']?.texts?.[0] ?? 'Combine skeletons to simulate a card with an avatar and text.'}</DocText>
        <Showcase
          className="flex-col items-stretch"
        ...
        <PropsTable
          data={[
            ...
            },
            {
              ...
              type: '"shimmer" | "pulse"',
              default: '"shimmer"',
              description: pt?.props?.['animation'] ?? 'Animation style. Shimmer sweeps a highlight, pulse fades opacity.',
            },
            {
              name: 'width',
              type: 'string | number',
