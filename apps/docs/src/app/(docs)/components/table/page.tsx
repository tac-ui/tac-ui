  ...
  return (
    <DocPage>
      <div>
        ...
        <DocDescription>{pt?.description ?? 'A composable table component for displaying structured tabular data.'}</DocDescription>
      </div>

      <DocSection title={pt?.sections?.['basic']?.title ?? 'Basic'}>
        <DocText>{pt?.sections?.['basic']?.texts?.[0] ?? 'A minimal table using TableHeader, TableBody, TableRow, TableHead, and TableCell. Rows gain a hover highlight by default.'}</DocText>
        <Showcase
  ...
  <TableBody>
    ...
    {/* ...more rows */}
      ...
      <DocSection title={pt?.sections?.['invoice-example']?.title ?? 'Invoice Example'}>
        <DocText>{pt?.sections?.['invoice-example']?.texts?.[0] ?? 'A complete table with header, body, footer for totals, a caption, and status badges — demonstrating all subcomponents together.'}</DocText>
        <Showcase
          className="flex-col items-stretch p-0 overflow-hidden"
          code={`<Table>
  ...
  <TableFooter>
    <TableRow>
      ...
      <TableCell className="text-right">$8,220.00</TableCell>
        ...
        >
          <Table>
            ...
            <TableFooter>
              <TableRow>
                <TableCell colSpan={4} className="font-semibold pb-1">Total</TableCell>
      ...
      <DocSection title={pt?.sections?.['with-caption']?.title ?? 'With Caption'}>
        <DocText>{pt?.sections?.['with-caption']?.texts?.[0] ?? 'TableCaption renders an accessible caption below the table, useful for labeling the data context.'}</DocText>
        <Showcase
          className="flex-col items-stretch p-0 overflow-hidden"
          code={`<Table>
  ...
  <TableBody>
    <TableRow>
      ...
      <DocSection title={pt?.sections?.['api-reference']?.title ?? 'API Reference'}>
        <DocText>{pt?.sections?.['api-reference']?.texts?.[0] ?? 'All subcomponents accept standard HTML element attributes (className, style, etc.) and forward their ref to the underlying DOM element.'}</DocText>
        <PropsTable
          data={[
            {
              name: 'Table',
            ...
            },
            {
              ...
              default: '-',
              description: pt?.props?.['TableHeader'] ?? 'Renders <thead>. Applies a subtle background and border-bottom to its rows.',
            },
            {
              name: 'TableBody',
              type: 'React.HTMLAttributes<HTMLTableSectionElement>',
              default: '-',
              description: pt?.props?.['TableBody'] ?? 'Renders <tbody>. Removes the border from the last row automatically.',
            },
            {
              name: 'TableFooter',
              type: 'React.HTMLAttributes<HTMLTableSectionElement>',
              default: '-',
              description: pt?.props?.['TableFooter'] ?? 'Renders <tfoot> with a top border and secondary background for totals or summaries.',
            },
            {
              name: 'TableRow',
              type: 'React.HTMLAttributes<HTMLTableRowElement>',
              default: '-',
              description: pt?.props?.['TableRow'] ?? 'Renders <tr> with hover highlight. Applies selected styling when data-state="selected".',
            },
            {
              name: 'TableHead',
              type: 'React.ThHTMLAttributes<HTMLTableCellElement>',
              default: '-',
              description: pt?.props?.['TableHead'] ?? 'Renders <th scope="col"> with muted foreground text and medium font weight.',
            },
            {
              name: 'TableCell',
              type: 'React.TdHTMLAttributes<HTMLTableCellElement>',
              default: '-',
              description: pt?.props?.['TableCell'] ?? 'Renders <td> with standard padding and middle vertical alignment.',
            },
            {
              name: 'TableCaption',
              type: 'React.HTMLAttributes<HTMLTableCaptionElement>',
              default: '-',
              description: pt?.props?.['TableCaption'] ?? 'Renders <caption> positioned below the table in muted foreground text.',
            },
          ]}
        />
      </DocSection>
