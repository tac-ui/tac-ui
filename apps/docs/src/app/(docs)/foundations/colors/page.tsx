...
function ColorSwatch({ cssVar, name }: { cssVar: string; name: string }) {
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div ref={ref} className="w-full h-12 rounded-[var(--radius-m)] border border-solid border-[var(--border)]" style={{ backgroundColor: `var(${cssVar})` }} />
...
const coreColors = [
  ['--background', 'background'], ['--background-subtle', 'bg-subtle'], ['--surface', 'surface'], ['--foreground', 'foreground'],
  ['--primary', 'primary'], ['--primary-hover', 'primary-hover'], ['--primary-foreground', 'primary-fg'],
  ...
  ['--point', 'point'], ['--point-hover', 'point-hover'], ['--point-foreground', 'point-fg'], ['--point-subtle', 'point-subtle']
];

const statusColors = [
  ['--success', 'success'], ['--success-bg', 'success-bg'], ['--success-foreground', 'success-fg'],
  ['--warning', 'warning'], ['--warning-bg', 'warning-bg'], ['--warning-foreground', 'warning-fg'],
  ['--error', 'error'], ['--error-bg', 'error-bg'], ['--error-foreground', 'error-fg'],
  ['--info', 'info'], ['--info-bg', 'info-bg'], ['--info-foreground', 'info-fg'],
];
  ...
  return (
    <DocPage>
      <div>
        ...
        <DocDescription>{pt?.description ?? 'Semantic color tokens that adapt automatically between light and dark themes.'}</DocDescription>
      </div>

      <DocSection title={pt?.sections?.['core-colors']?.title ?? 'Core Colors'}>
        <DocText>{pt?.sections?.['core-colors']?.texts?.[0] ?? 'Primary surface, text, and accent colors used throughout the system.'}</DocText>
        <ColorGrid colors={coreColors} />
      </DocSection>

      <DocSection title={pt?.sections?.['status-colors']?.title ?? 'Status Colors'}>
        <DocText>{pt?.sections?.['status-colors']?.texts?.[0] ?? 'Semantic colors for success, warning, error, and informational states.'}</DocText>
        <ColorGrid colors={statusColors} />
      </DocSection>

      <DocSection title={pt?.sections?.['gray-scale']?.title ?? 'Gray Scale'}>
        <DocText>{pt?.sections?.['gray-scale']?.texts?.[0] ?? 'Neutral palette from 50 (lightest) to 900 (darkest).'}</DocText>
      ...
      <DocSection title={pt?.sections?.['chart-colors']?.title ?? 'Chart Colors'}>
        <DocText>{pt?.sections?.['chart-colors']?.texts?.[0] ?? '8-color palette optimized for data visualization.'}</DocText>
      ...
      <DocSection title={pt?.sections?.['usage']?.title ?? 'Usage'}>
        <DocText>{pt?.sections?.['usage']?.texts?.[0] ?? 'Access colors as CSS custom properties or Tailwind utility classes.'}</DocText>
        <div className="rounded-[var(--radius-lg)] border border-solid border-[var(--border)] overflow-hidden">
          <div className="bg-[var(--card)] text-[var(--foreground)] p-4 font-mono text-[13px] leading-relaxed">
            <p className="text-[var(--muted-foreground)]">{/* CSS variable */}{'/* CSS variable */'}</p>
            <p>{`color: var(--primary);`}</p>
            <p>{`background: var(--card);`}</p>
            <br />
            <p className="text-[var(--muted-foreground)]">{/* Tailwind class */}{'/* Tailwind class */'}</p>
