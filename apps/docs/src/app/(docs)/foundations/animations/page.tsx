...
import { cn, Button, Card, CardHeader, CardTitle, CardContent, AnimatedToggle } from '@tac-ui/web';
import { Play, RotateCcw, Sun, Moon } from '@tac-ui/icon';
import { usePageTranslation } from '@/i18n';
import { DocPage, DocTitle, DocDescription, DocSection, DocText, InlineCode, Showcase } from '@/components/docs/DocPage';

/* ─── Spring Presets ─── */
const diaSpringPresets = [
...
function SpringDemo({ preset }: { preset: typeof diaSpringPresets[0] }) {
  ...
  return (
    <div className="flex flex-col gap-3 p-4 rounded-[var(--radius-lg)] border border-solid border-[var(--border)] bg-[var(--background)]">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm font-semibold text-[var(--foreground)]">diaSpring.{preset.name}</p>
...
/* ─── Motion Variants ─── */
const diaSpringDefault = { type: 'spring' as const, stiffness: 260, damping: 34, mass: 1 };
const diaSpringMagnetic = { type: 'spring' as const, stiffness: 340, damping: 38, mass: 0.8 };
const diaSpringEntrance = { type: 'spring' as const, stiffness: 180, damping: 28, mass: 1.2 };
const diaSpringHeavy = { type: 'spring' as const, stiffness: 220, damping: 32, mass: 1.5 };
const diaSpringLight = { type: 'spring' as const, stiffness: 260, damping: 34, mass: 0.5 };
...
const variantDemos = [
  {
    ...
    variants: {
      hidden: { opacity: 0, scale: 0.96, y: -4, transition: diaSpringMagnetic },
      visible: { opacity: 1, scale: 1, y: 0, transition: diaSpringMagnetic },
    },
  },
  {
    name: 'panelVariants',
    ...
    variants: {
      ...
      visible: { opacity: 1, scale: 1, filter: 'blur(0px)', transition: diaSpringHeavy },
    },
  },
  {
    ...
    variants: {
      ...
      visible: { opacity: 1, filter: 'blur(0px)', y: 0, transition: diaSpringEntrance },
    },
  },
  {
    ...
    variants: {
      ...
      visible: { opacity: 1, y: 0, filter: 'blur(0px)', transition: diaSpringMagnetic },
    },
  },
];
  ...
  return (
    <DocPage>
      <div>
        ...
        <DocDescription>
          {pt?.description ?? 'Motion tokens, spring physics, and interaction hooks that give Tac UI its organic, physical feel.'}
        </DocDescription>
      </div>

      <DocSection title={pt?.sections?.['philosophy']?.title ?? 'Philosophy'}>
        <DocText>
          {pt?.sections?.['philosophy']?.texts?.[0] ?? <>Tac UI uses spring-based animations inspired by Dia Browser. Elements have mass and momentum — they spring
          into place with natural deceleration rather than sliding mechanically. The system is built on Framer Motion
          springs with five mass-differentiated presets called <InlineCode>diaSpring</InlineCode>.</>}
        </DocText>
      </DocSection>

      <DocSection title={pt?.sections?.['spring-presets']?.title ?? 'Spring Presets'}>
        <DocText>
          {pt?.sections?.['spring-presets']?.texts?.[0] ?? <>Import via <InlineCode>{'import { diaSpring } from \'@tac-ui/tokens\''}</InlineCode>. Each preset varies in mass,
          creating distinct physical characters.</>}
        </DocText>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {diaSpringPresets.map((p) => (
      ...
      <DocSection title={pt?.sections?.['duration-scale']?.title ?? 'Duration Scale'}>
        <DocText>
          {pt?.sections?.['duration-scale']?.texts?.[0] ?? <>CSS transition durations for non-spring animations. Access via <InlineCode>motion.duration</InlineCode>.</>}
        </DocText>
          ...
          {durations.map((d) => (
            <div key={d.name} className="flex items-center gap-4 py-2">
              ...
              <div className="flex-1 h-2 rounded-full bg-[var(--secondary)] overflow-hidden">
                <div
                  className="h-full rounded-full bg-[var(--point)]"
                  style={{ width: `${(d.ms / 450) * 100}%` }}
                />
              </div>
      ...
      <DocSection title={pt?.sections?.['easing-curves']?.title ?? 'Easing Curves'}>
        <DocText>
          {pt?.sections?.['easing-curves']?.texts?.[0] ?? <>CSS easing functions for transitions. Access via <InlineCode>motion.easing</InlineCode>.</>}
        </DocText>
      ...
      <DocSection title={pt?.sections?.['motion-variants']?.title ?? 'Motion Variants'}>
        <DocText>
          {pt?.sections?.['motion-variants']?.texts?.[0] ?? <>Pre-built Framer Motion variant objects for common animation patterns. Import
          from <InlineCode>@tac-ui/web</InlineCode> internal constants.</>}
        </DocText>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {variantDemos.map((d) => (
      ...
      <DocSection title={pt?.sections?.['interactive-hover']?.title ?? 'Interactive Hover'}>
        <DocText>
          {pt?.sections?.['interactive-hover']?.texts?.[0] ?? 'Spring-based hover and tap effects for interactive elements. Click or hover to feel the physics.'}
        </DocText>
        <Showcase code={`// interactiveHover — buttons, cards, pagination
whileHover={{ scale: 1.02, y: -1 }}
whileTap={{ scale: 0.97, y: 0 }}
...
whileTap={{ scale: 0.98 }}`}>
      ...
      <DocSection title={pt?.sections?.['animated-toggle']?.title ?? 'AnimatedToggle'}>
        <DocText>
          {pt?.sections?.['animated-toggle']?.texts?.[0] ?? <>A button that swaps icons with a rotation animation. Uses <InlineCode>diaSpring.light</InlineCode> for
...
/>`}>
      ...
      <DocSection title={pt?.sections?.['interaction-hooks']?.title ?? 'Interaction Hooks'}>
        <DocText>
          {pt?.sections?.['interaction-hooks']?.texts?.[0] ?? <>Hooks for cursor-tracking effects. These power the Card component{"'"}s interactive features
