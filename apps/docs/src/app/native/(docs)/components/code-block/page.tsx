...
const tsExample = `import { Badge } from '@tac-ui/native';
...
npm install @tac-ui/native

# Or with pnpm
pnpm add @tac-ui/native

# Start the dev server
npm run dev`;

const glassExample = `import { CodeBlock } from '@tac-ui/native';
...
export default function NativeCodeBlockPage() {
  const pt = usePageTranslation('native-code-block');
  return (
    <DocPage navGroups={nativeNavGroups}>
      ...
      <DocSection title="Import">
        <PreviewCode code={`import { CodeBlock } from '@tac-ui/native';`} />
      </DocSection>

      <DocSection title={pt?.sections?.['playground']?.title ?? 'Playground'}>
        <DocText>
          {pt?.sections?.['playground']?.texts?.[0] ?? 'Interactively configure the CodeBlock props below.'}
        </DocText>
        <NativePlayground
          controls={{
            language: {
          ...
          render={(values) => {
            const codeByLang: Record<string, string> = {
              tsx: `import { Button } from '@tac-ui/native';\n\nexport function Demo() {\n  return <Button variant="primary">Click me</Button>;\n}`,
              bash: `# Install tac-ui\nnpm install @tac-ui/native\n\n# Start the dev server\nnpm run dev`,
              json: `{\n  "name": "@tac-ui/native",\n  "version": "1.0.0",\n  "description": "Tac UI Native Components"\n}`,
            };
            return (
              <CodeBlock
        ...
        <PropsTable
          data={[
            ...
            },
            {
              ...
              type: 'boolean',
              default: 'false',
              description: pt?.props?.['glass'] ?? 'When true, applies a semi-transparent background.',
            },
          ]}
        />
      </DocSection>
