...
export default function AsymmetricExample() {
        ...
        <article className="flex flex-col gap-6">
          <div className="flex flex-col gap-3">
            ...
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-full bg-[var(--secondary)] flex items-center justify-center text-sm font-medium">
                J
              </div>
              <div>
          ...
          <div className="flex flex-col gap-4 text-sm leading-relaxed">
            <p className="m-0">
              Flexbox is one of the most powerful CSS layout models available today. It provides a flexible way to
              arrange items in a container, making it easy to create responsive designs without relying on floats or
              positioning hacks.
            </p>
            <h2 className="text-xl font-semibold m-0 mt-2">Understanding the Basics</h2>
            <p className="m-0">
              The flex container is the parent element that holds flex items. By setting{' '}
              <code className="px-1.5 py-0.5 rounded bg-[var(--secondary)] text-xs">display: flex</code>, you transform
              its children into flex items that can be arranged along a main axis and cross axis.
            </p>
  ...
  gap: 1rem;
            ...
            <p className="m-0">
              Flexbox excels at creating navigation bars, card layouts, centering content, and distributing space among
              items. Understanding the key properties like{' '}
              <code className="px-1.5 py-0.5 rounded bg-[var(--secondary)] text-xs">justify-content</code>,{' '}
              <code className="px-1.5 py-0.5 rounded bg-[var(--secondary)] text-xs">align-items</code>, and{' '}
              <code className="px-1.5 py-0.5 rounded bg-[var(--secondary)] text-xs">flex-wrap</code> is essential for
              mastering this layout model.
            </p>
          </div>
        </article>
      }
        ...
        <div className="flex flex-col gap-6">
            ...
            <div className="flex gap-2">
              {[
                ...
                return (
                  <div
                    key={a.label}
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-[var(--radius-m)] border border-solid border-[var(--border)] text-xs cursor-pointer hover:bg-[var(--secondary)] transition-colors"
                  >
                    <Icon size={12} />
                    {a.label}
                  </div>
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex flex-wrap gap-1.5">
              {['CSS', 'Layout', 'Flexbox', 'Responsive', 'Frontend'].map((tag) => (
                <span
                  key={tag}
                  className="text-xs px-2 py-1 rounded-full bg-[var(--secondary)] text-[var(--muted-foreground)] cursor-pointer hover:text-[var(--foreground)] transition-colors"
                >
                  {tag}
                </span>
              ))}
          ...
          <div className="flex flex-col gap-3">
            ...
            {relatedArticles.map((article) => (
              <div
                key={article.title}
                className="flex items-center justify-between py-2 border-b border-solid border-[var(--border)] last:border-0 cursor-pointer group"
              >
