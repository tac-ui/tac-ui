  ...
  return (
      ...
      header={
        <Header bordered sticky>
          ...
          <div className="ml-auto flex items-center gap-4 text-sm">
            <span className="text-[var(--muted-foreground)] cursor-pointer hover:text-[var(--foreground)] transition-colors">
              Features
            </span>
            <span className="text-[var(--muted-foreground)] cursor-pointer hover:text-[var(--foreground)] transition-colors">
              Pricing
    ...
    >
      ...
      <section className="py-20 px-6 flex flex-col items-center text-center bg-gradient-to-b from-[var(--secondary)] to-[var(--background)]">
        <span className="text-xs font-medium px-3 py-1 rounded-full bg-[var(--primary)]/10 text-[var(--primary)] mb-4">
          New Release
        </span>
        <h1 className="text-4xl md:text-5xl font-bold m-0 max-w-4xl leading-tight">
          Build products faster with our platform
        </h1>
        <p className="text-lg text-[var(--muted-foreground)] m-0 mt-4 max-w-3xl">
          Streamline your workflow with powerful tools designed for modern teams.
        </p>
        <div className="flex gap-3 mt-8">
          <div className="px-6 py-2.5 rounded-[var(--radius-m)] bg-[var(--primary)] text-[var(--primary-foreground)] text-sm font-medium cursor-pointer">
            Get Started
          </div>
          <div className="px-6 py-2.5 rounded-[var(--radius-m)] border border-solid border-[var(--border)] text-sm font-medium cursor-pointer hover:bg-[var(--secondary)] transition-colors">
            Learn More
          </div>
        </div>
      </section>
