          ...
          <SidebarGroup label="Members">
            {members.map((m) => (
              <SidebarItem key={m.name}>
                ...
                <div
                  className={`w-2 h-2 rounded-full ${m.status === 'online' ? 'bg-green-500' : m.status === 'away' ? 'bg-yellow-500' : 'bg-[var(--border)]'}`}
                />
              </SidebarItem>
            ))}
          </SidebarGroup>
        ...
        {[
          {
            user: 'Alice',
            time: '10:32 AM',
            msg: 'Good morning everyone! Quick reminder about the all-hands meeting at 2pm today.',
          },
          { user: 'Bob', time: '10:45 AM', msg: "Thanks Alice! I'll prepare the Q3 report slides." },
          {
            user: 'Carol',
            time: '11:02 AM',
            msg: "Has anyone reviewed the new design mockups? I'd love to get some feedback before the meeting.",
          },
        ].map((m, i) => (
          <div key={i} className="flex gap-3">
            <div className="w-8 h-8 rounded-full bg-[var(--secondary)] flex items-center justify-center text-sm font-medium shrink-0">
              {m.user[0]}
            </div>
            <div className="flex flex-col gap-1">
