  ...
  return (
    <DocPage>
      ...
      <DocSection title="Import">
        <PreviewCode code={`import { SegmentController } from '@tac-ui/web';`} />
      </DocSection>
...
<SegmentController options={options} size="lg" />`}
  ...
  { value: 'board', label: 'Board', icon: <LayoutGrid size={14} /> },
      ...
      <DocSection title={pt?.sections?.['multi-select']?.title ?? 'Multi Select'}>
        <DocText>
          {pt?.sections?.['multi-select']?.texts?.[0] ??
        ...
        <Showcase
          code={`<SegmentController mode="multi" options={[
  { value: 'bold', label: 'Bold', icon: <Bold size={14} /> },
  { value: 'italic', label: 'Italic', icon: <Italic size={14} /> },
  { value: 'underline', label: 'Underline', icon: <Underline size={14} /> },
      ...
      <DocSection title={pt?.sections?.['multi-select-controlled']?.title ?? 'Multi Select (Controlled)'}>
        <DocText>
          {pt?.sections?.['multi-select-controlled']?.texts?.[0] ??
...
<SegmentController mode="multi" options={options} value={value} onChange={setValue} />`}
      ...
      <DocSection title={pt?.sections?.['full-width']?.title ?? 'Full Width'}>
        <DocText>
          {pt?.sections?.['full-width']?.texts?.[0] ??
  ...
  { value: 'monthly', label: 'Monthly', disabled: true },
      ...
      <DocSection title={pt?.sections?.['collapsible']?.title ?? 'Collapsible'}>
        <DocText>
          {pt?.sections?.['collapsible']?.texts?.[0] ??
        ...
        <PropsTable
          data={[
            ...
            },
            {
              ...
              description:
                pt?.props?.['mode'] ??
                'Selection mode. Single-select uses a sliding indicator; multi-select allows toggling multiple options with check icons.',
            },
            {
              name: 'value',
              type: 'string | string[]',
              ...
              description:
                pt?.props?.['value'] ??
                'Controlled selected value(s). String for single mode, string[] for multi mode.',
            },
            {
              name: 'defaultValue',
              type: 'string | string[]',
              default: '-',
              description:
                pt?.props?.['defaultValue'] ??
                'Initial value(s) for uncontrolled usage. Defaults to the first option in single mode.',
            },
            {
              name: 'onChange',
              ...
              description:
                pt?.props?.['onChange'] ??
                'Called when selection changes. Receives string for single mode, string[] for multi mode.',
            },
            {
              name: 'size',
              type: '"sm" | "md" | "lg"',
            ...
            },
            {
              ...
              default: 'false',
              description:
                pt?.props?.['fullWidth'] ?? 'When true, segments expand equally to fill the container width.',
            },
            {
              name: 'disabled',
              type: 'boolean',
            ...
            },
            {
              ...
              default: 'false',
              description:
                pt?.props?.['collapsible'] ??
                'When true, only the selected option is visible; expands on hover to reveal all options.',
            },
            {
              name: 'className',
              type: 'string',
              default: '-',
              description: pt?.props?.['className'] ?? 'Additional CSS class names applied to the root container.',
            },
          ]}
        />
      </DocSection>
