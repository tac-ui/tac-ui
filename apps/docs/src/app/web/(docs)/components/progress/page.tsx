  ...
  return (
    <DocPage>
      ...
      <DocSection title="Import">
        <PreviewCode code={`import { Progress } from '@tac-ui/web';`} />
      </DocSection>

      <DocSection title="Playground">
        <DocText>Interactively configure the Progress props below.</DocText>
        <Playground
          controls={{
            variant: {
              type: 'select',
          ...
          render={(values) => {
            ...
            const numValue = parseInt(values.value as string, 10);
            const showLabel = values.showLabel as boolean;
            if (variant === 'circular') {
              return <Progress variant={variant} value={numValue} size={64} showLabel={showLabel} />;
            }
            return <Progress variant={variant} value={numValue} showLabel={showLabel} className="w-full" />;
          }}
          code={(values) => {
