...
import { Input, Toggle } from '@tac-ui/web';
  ...
  return (
    <DocPage>
      <div>
        ...
        <DocDescription>
          {pt?.description ?? 'A text input field with optional label, helper text, icons, and error state support.'}
        </DocDescription>
      </div>

      <DocSection title="Import">
        <PreviewCode code={`import { Input } from '@tac-ui/web';`} />
      </DocSection>
...
<Input size="lg" placeholder="Large" />`}
      ...
      <DocSection title={pt?.sections?.['password-toggle']?.title ?? 'Password Toggle'}>
        <DocText>
          {pt?.sections?.['password-toggle']?.texts?.[0] ??
...
/>`}
      ...
      <DocSection title={pt?.sections?.['with-button']?.title ?? 'With Button'}>
        <DocText>
          {pt?.sections?.['with-button']?.texts?.[0] ??
        ...
        <Showcase
          className="flex-col items-stretch"
          code={`<Input label="Nickname" placeholder="Enter nickname" rightButton={<button>Check</button>} />`}
        >
        ...
        <PropsTable
          data={[
            ...
            },
            {
              ...
              type: 'React.ReactNode',
              default: '-',
              description: pt?.props?.['rightButton'] ?? 'Button element rendered inside the right side of the input.',
            },
            {
              name: 'disabled',
              type: 'boolean',
              default: 'false',
              description: pt?.props?.['disabled'] ?? 'Disables the input when true.',
            },
          ]}
        />
      </DocSection>
