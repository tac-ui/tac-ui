  ...
  return (
    <DocPage>
      ...
      <DocSection title="Import">
        <PreviewCode code={`import { Select } from '@tac-ui/web';`} />
      </DocSection>
      ...
      <DocSection title={pt?.sections?.['default']?.title ?? 'Default'}>
        ...
        <Showcase
          ...
          code={`<Select options={options} placeholder="Choose an option" />`}
        >
      ...
      <DocSection title={pt?.sections?.['sizes']?.title ?? 'Sizes'}>
        <DocText>
          {pt?.sections?.['sizes']?.texts?.[0] ??
            'Three size variants are available via selectSize: sm (32px), md (40px, default), and lg (48px). Font size scales accordingly.'}
        </DocText>
        <Showcase
...
<Select options={options} label="Large" placeholder="Choose an option" selectSize="lg" />`}
      ...
      <DocSection title={pt?.sections?.['disabled']?.title ?? 'Disabled'}>
        ...
        <Showcase
          className="flex-col items-stretch"
          code={`<Select options={options} placeholder="Disabled select" disabled />`}
        >
