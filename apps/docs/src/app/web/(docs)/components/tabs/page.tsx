...
export default function TabsPage() {
  ...
  return (
    <DocPage>
      ...
      <DocSection title="Import">
        <PreviewCode code={`import { Tabs, TabsList, TabTrigger, TabContent } from '@tac-ui/web';`} />
      </DocSection>

      <DocSection title="Playground">
        <DocText>Interactively configure the Tabs props below.</DocText>
        <Playground
          controls={{
            variant: {
              type: 'select',
              label: 'Variant',
              options: ['underline', 'pill', 'outline', 'icon'],
              defaultValue: 'underline',
            },
          }}
          render={(values) => (
            <Tabs defaultValue="tab1" variant={values.variant as 'underline' | 'pill' | 'outline' | 'icon'}>
              <TabsList>
  ...
  <TabContent value="tab3">General settings content.</TabContent>
      ...
      <DocSection title={pt?.sections?.['icon']?.title ?? 'Icon'}>
        ...
        <Showcase
          code={`<Tabs defaultValue="tab1" variant="icon">
  <TabsList>
  ...
  <TabContent value="tab3">Notification preferences content.</TabContent>
        ...
        >
          <Tabs defaultValue="tab1" variant="icon">
            <TabsList>
              <TabTrigger value="tab1" icon={<User />}>
                Account
              </TabTrigger>
              <TabTrigger value="tab2" icon={<Settings />}>
                Settings
              </TabTrigger>
              <TabTrigger value="tab3" icon={<Bell />}>
                Notifications
              </TabTrigger>
            </TabsList>
            <TabContent value="tab1">Account settings content.</TabContent>
      ...
      <DocSection title={pt?.sections?.['api-reference']?.title ?? 'API Reference'}>
        ...
        <PropsTable
          data={[
            {
              name: 'variant',
              type: '"underline" | "pill" | "outline" | "icon"',
              default: '"underline"',
              description: pt?.props?.['variant'] ?? 'Visual style variant of the tab list.',
            },
            {
              name: 'defaultValue',
              type: 'string',
              default: '-',
              description: pt?.props?.['defaultValue'] ?? 'The tab value open by default in uncontrolled mode.',
            },
            {
              name: 'value',
              type: 'string',
              default: '-',
              description: pt?.props?.['value'] ?? 'The controlled active tab value.',
            },
            {
              name: 'onValueChange',
            ...
            },
            {
              ...
              default: '-',
              description: pt?.props?.['children'] ?? 'Compose with TabsList, TabTrigger, and TabContent.',
            },
          ]}
        />
          ...
          data={[
            {
              ...
              default: '-',
              description:
                pt?.props?.['tabTriggerValue'] ?? 'The value this trigger activates; must match a TabContent value.',
            },
            {
              name: 'icon',
              type: 'React.ReactNode',
              default: '-',
              description:
                pt?.props?.['tabTriggerIcon'] ??
                'Icon element to display (used with icon variant; shown collapsed on inactive tabs).',
            },
            {
              name: 'children',
              type: 'React.ReactNode',
              default: '-',
              description: pt?.props?.['tabTriggerChildren'] ?? 'Tab label text.',
            },
          ]}
        />
        ...
        <PropsTable
          data={[
            ...
            },
            {
              ...
              type: 'React.ReactNode',
              default: '-',
              description: pt?.props?.['tabContentChildren'] ?? 'Content rendered when this tab is active.',
            },
            {
              name: 'className',
              type: 'string',
              default: '-',
              description: pt?.props?.['tabContentClassName'] ?? 'Additional CSS class names.',
            },
          ]}
        />
      </DocSection>
