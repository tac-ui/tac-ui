  ...
  return (
    <DocPage>
      ...
      <DocSection title="Import">
        <PreviewCode code={`import { Divider } from '@tac-ui/web';`} />
      </DocSection>

      <DocSection title="Playground">
        <DocText>Interactively configure the Divider props below.</DocText>
        <Playground
          controls={{
            variant: {
              type: 'select',
          ...
          render={(values) => (
            <div className="w-full">
              <Divider
                variant={values.variant as 'full' | 'inset' | 'thick' | 'withLabel'}
                label={values.variant === 'withLabel' ? 'OR' : undefined}
              />
            </div>
          )}
          code={(values) =>
            values.variant === 'withLabel' ? `<Divider label="OR" />` : `<Divider variant="${values.variant}" />`
          }
        />
      </DocSection>
