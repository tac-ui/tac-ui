...
function ToastPreview({ variant }: { variant: string }) {
  return (
    <div
      className={`flex items-center gap-3 pl-5 pr-4 py-3.5 min-w-[320px] max-w-[480px] w-auto rounded-[var(--radius-lg)] ${variantStyles[variant]}`}
    >
  ...
  Show Toast
        ...
        >
          <div className="flex flex-wrap gap-2">
            <Button variant="secondary" onClick={() => toast('This is a default toast message')}>
              Default
            </Button>
            <Button variant="secondary" onClick={() => toast('Your changes have been saved!', { variant: 'success' })}>
              Success
            </Button>
            <Button variant="secondary" onClick={() => toast('Something went wrong.', { variant: 'error' })}>
              Error
            </Button>
            <Button
              variant="secondary"
              onClick={() => toast('Your session expires in 5 minutes.', { variant: 'warning' })}
            >
              Warning
            </Button>
            <Button variant="secondary" onClick={() => toast('A new update is available.', { variant: 'info' })}>
              Info
            </Button>
          </div>
        </Showcase>
  ...
  return (
    <DocPage>
      <div>
        ...
        <DocDescription>
          {pt?.description ?? 'A brief, auto-dismissing notification displayed at the edge of the screen.'}
        </DocDescription>
      </div>

      <DocSection title="Import">
        <PreviewCode code={`import { useToast } from '@tac-ui/web';`} />
      </DocSection>
      ...
      <DocSection title={pt?.sections?.['api-reference']?.title ?? 'API Reference'}>
        <DocText>
          {pt?.sections?.['api-reference']?.texts?.[0] ?? 'Props for the toast() function returned by useToast().'}
        </DocText>
        <PropsTable
          data={[
            {
              ...
              type: 'string',
              default: '-',
              description: pt?.props?.['message'] ?? 'The text content of the toast notification.',
            },
            {
              name: 'options.variant',
              type: '"default" | "success" | "error" | "warning" | "info"',
              default: '"default"',
              description: pt?.props?.['options.variant'] ?? 'Visual style variant of the toast.',
            },
            {
            ...
            },
            {
              ...
              default: '-',
              description:
                pt?.props?.['options.action'] ?? 'Optional action element rendered to the right of the message.',
            },
            {
              name: 'options.icon',
              type: 'React.ReactNode',
              default: '-',
              description:
                pt?.props?.['options.icon'] ?? 'Optional icon rendered on the left (visible in default variant only).',
            },
          ]}
        />
      </DocSection>
      ...
      <DocSection title={pt?.sections?.['toast-provider-props']?.title ?? 'ToastProvider Props'}>
        <DocText>
          {pt?.sections?.['toast-provider-props']?.texts?.[0] ??
          ...
          data={[
            {
              ...
              description: pt?.props?.['position'] ?? 'Screen position where the toast stack is anchored.',
            },
            {
              name: 'maxToasts',
              type: 'number',
              ...
              description:
                pt?.props?.['maxToasts'] ??
                'Maximum number of toasts visible at once. Oldest toasts are trimmed when the limit is exceeded.',
            },
            {
              name: 'children',
              type: 'React.ReactNode',
              default: '-',
              description: pt?.props?.['providerChildren'] ?? 'Application content to wrap with the toast context.',
            },
          ]}
        />
      </DocSection>
