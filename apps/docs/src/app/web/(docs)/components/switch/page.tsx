...
function ControlledDemo() {
  ...
  return <Switch checked={checked} onChange={(val) => setChecked(val)} />;
}
  ...
  return (
    <DocPage>
      ...
      <DocSection title="Import">
        <PreviewCode code={`import { Switch } from '@tac-ui/web';`} />
      </DocSection>

      <DocSection title="Playground">
        <DocText>Interactively configure the Switch props below.</DocText>
        <Playground
          controls={{
            label: {
              type: 'text',
      ...
      <DocSection title={pt?.sections?.['disabled']?.title ?? 'Disabled'}>
        <DocText>
          {pt?.sections?.['disabled']?.texts?.[0] ??
            'Disabled switches are non-interactive, visually dimmed, and desaturated regardless of their current state.'}
        </DocText>
        <Showcase
      ...
      <DocSection title={pt?.sections?.['disabled-with-label']?.title ?? 'Disabled with Label'}>
        <DocText>
          {pt?.sections?.['disabled-with-label']?.texts?.[0] ??
            'Labeled switches can also be disabled, shown at reduced opacity and with desaturation to indicate non-interactive state.'}
        </DocText>
        <Showcase
        ...
        <PropsTable
          data={[
            ...
            },
            {
              ...
              description:
                pt?.props?.['className'] ??
                'Additional CSS class names. Applied to the labeled wrapper when label is set, otherwise to the button.',
            },
          ]}
        />
      </DocSection>
