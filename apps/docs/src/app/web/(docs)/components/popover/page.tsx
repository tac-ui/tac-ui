  ...
  return (
    <DocPage>
      <div>
        ...
        <DocDescription>
          {pt?.description ?? 'A floating panel anchored to a trigger element, used for richer content than a tooltip.'}
        </DocDescription>
      </div>

      <DocSection title="Import">
        <PreviewCode code={`import { Popover, PopoverHeader, PopoverBody, PopoverFooter } from '@tac-ui/web';`} />
      </DocSection>

      <DocSection title={pt?.sections?.['basic-popover']?.title ?? 'Basic Popover'}>
        <DocText>
          {pt?.sections?.['basic-popover']?.texts?.[0] ??
            'Click the trigger to open a floating panel. The popover closes on outside click or Escape key.'}
        </DocText>
        <Showcase
          code={`<Popover trigger={<Button variant="outline">Open Popover</Button>}>
  <PopoverBody>
      ...
      <button className="...">Option 3</button>
        ...
        >
          <Popover trigger={<Button variant="outline">Menu</Button>}>
            <PopoverBody className="p-0">
              <div className="flex flex-col py-1">
                <button className="px-4 py-2 text-left text-sm bg-transparent border-none cursor-pointer text-[var(--foreground)] hover:bg-[var(--interactive-hover)] transition-colors">
                  Option 1
                </button>
                <button className="px-4 py-2 text-left text-sm bg-transparent border-none cursor-pointer text-[var(--foreground)] hover:bg-[var(--interactive-hover)] transition-colors">
                  Option 2
          ...
          data={[
            {
              ...
              default: '-',
              description: pt?.props?.['trigger'] ?? 'The element that toggles the popover open/closed when clicked.',
            },
            {
              name: 'open',
              type: 'boolean',
            ...
            },
            {
              ...
              default: '"bottom"',
              description: pt?.props?.['side'] ?? 'Side on which the panel appears relative to the trigger.',
            },
            {
              name: 'align',
              type: '"start" | "center" | "end"',
              default: '"center"',
              description: pt?.props?.['align'] ?? 'Alignment of the panel along the perpendicular axis.',
            },
            {
              name: 'children',
              type: 'React.ReactNode',
              default: '-',
              description: pt?.props?.['children'] ?? 'Content rendered inside the popover panel.',
            },
          ]}
        />
      </DocSection>
