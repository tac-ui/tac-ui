  ...
  return (
    <DocPage>
      ...
      <DocSection title="Import">
        <PreviewCode
          code={`import { Drawer, DrawerHeader, DrawerTitle, DrawerDescription, DrawerBody, DrawerFooter } from '@tac-ui/web';`}
        />
      </DocSection>

      <DocSection title={pt?.sections?.['sides']?.title ?? 'Sides'}>
        <DocText>
          {pt?.sections?.['sides']?.texts?.[0] ??
  ...
  <DrawerFooter>
    <Button onClick={() => setOpen(false)}>Close</Button>
      ...
      <DocSection title={pt?.sections?.['with-content']?.title ?? 'With Content'}>
        <DocText>
          {pt?.sections?.['with-content']?.texts?.[0] ??
          ...
          data={[
            {
              ...
              type: 'boolean',
              default: 'false',
              description: pt?.props?.['open'] ?? 'Controls whether the drawer is visible.',
            },
            {
              name: 'onClose',
              type: '() => void',
              default: '-',
              description:
                pt?.props?.['onClose'] ?? 'Callback fired when the drawer should close (backdrop click or Escape key).',
            },
            {
              name: 'side',
              type: '"left" | "right" | "top" | "bottom"',
              default: '"right"',
              description: pt?.props?.['side'] ?? 'Edge of the screen the drawer slides in from.',
            },
            {
              name: 'backdrop',
              type: 'boolean',
            ...
            },
            {
              ...
              description:
                pt?.props?.['children'] ??
                'Compose with DrawerHeader, DrawerTitle, DrawerDescription, DrawerBody, DrawerFooter.',
            },
            {
              name: 'className',
              type: 'string',
              default: '-',
              description: pt?.props?.['className'] ?? 'Additional CSS class names applied to the drawer panel.',
            },
          ]}
        />
      </DocSection>
