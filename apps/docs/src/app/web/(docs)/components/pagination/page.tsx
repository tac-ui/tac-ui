...
export default function PaginationPage() {
  ...
  return (
    <DocPage>
      ...
      <DocSection title="Import">
        <PreviewCode
          code={`import { Pagination, PaginationContent, PaginationItem, PaginationEllipsis, PaginationPrevious, PaginationNext } from '@tac-ui/web';`}
        />
      </DocSection>

      <DocSection title="Playground">
        <DocText>Interactively configure the Pagination props below.</DocText>
        <Playground
          controls={{
            totalPages: {
              type: 'select',
          ...
          render={(values) => {
            const total = Number(values.totalPages);
            const current = Number(values.currentPage);
            ...
            return (
              <Pagination>
                <PaginationContent>
                  ...
                  {total > 5 && <PaginationEllipsis />}
                  {total > 5 && <PaginationItem>{total}</PaginationItem>}
                  <PaginationNext />
                </PaginationContent>
              </Pagination>
            );
  ...
  <PaginationContent>
    ...
    <PaginationItem active>${values.currentPage}</PaginationItem>
    <PaginationItem>${values.totalPages}</PaginationItem>
    <PaginationNext />
  </PaginationContent>
</Pagination>`
          }
  ...
  <PaginationContent>
    ...
    <PaginationNext />
      ...
      <DocSection title={pt?.sections?.['with-ellipsis']?.title ?? 'With Ellipsis'}>
        <DocText>
          {pt?.sections?.['with-ellipsis']?.texts?.[0] ??
          ...
          data={[
            {
              ...
              default: '"Pagination"',
              description:
                pt?.props?.['label'] ?? 'Accessible label for the pagination nav element, used by screen readers.',
            },
            {
              name: 'className',
              type: 'string',
          ...
          data={[
            {
              ...
              type: 'boolean',
              default: 'false',
              description: pt?.props?.['glass'] ?? 'Applies glassmorphism styling to the pagination container.',
            },
            {
              name: 'className',
              type: 'string',
          ...
          data={[
            {
              ...
              description:
                pt?.props?.['active'] ??
                'Highlights this item as the currently active page with a point-colored background.',
            },
            {
              name: 'disabled',
              type: 'boolean',
              default: 'false',
              description:
                pt?.props?.['disabled'] ?? 'Disables the page button, reducing opacity and blocking interaction.',
            },
            {
              name: 'children',
              type: 'React.ReactNode',
              default: '-',
              description: pt?.props?.['children'] ?? 'Page number or label rendered inside the button.',
            },
          ]}
        />
        <DocText>
          {pt?.sections?.['api-reference']?.texts?.[3] ??
