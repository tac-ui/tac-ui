  ...
  return (
    <DocPage>
      <div>
        ...
        <DocDescription>
          {pt?.description ?? "Shows the user's current location within the application's navigation hierarchy."}
        </DocDescription>
      </div>

      <DocSection title="Import">
        <PreviewCode
          code={`import { Breadcrumb, BreadcrumbList, BreadcrumbItem, BreadcrumbLink, BreadcrumbSeparator, BreadcrumbEllipsis } from '@tac-ui/web';`}
        />
      </DocSection>

      <DocSection title={pt?.sections?.['simple']?.title ?? 'Simple'}>
        <DocText>
          {pt?.sections?.['simple']?.texts?.[0] ??
  ...
  <BreadcrumbList>
    ...
    <BreadcrumbItem current>
      ...
      <DocSection title={pt?.sections?.['with-current-item']?.title ?? 'With Current Item'}>
        <DocText>
          {pt?.sections?.['with-current-item']?.texts?.[0] ??
          ...
          data={[
            {
              ...
              default: 'false',
              description:
                pt?.props?.['BreadcrumbItem.current'] ??
                'Marks this item as the current page, applying active styles and aria-current="page".',
            },
            {
              name: 'BreadcrumbItem.children',
              type: 'React.ReactNode',
              default: '-',
              description:
                pt?.props?.['BreadcrumbItem.children'] ??
                'Content of the breadcrumb item (link, ellipsis, or plain text).',
            },
            {
              ...
              default: '-',
              description:
                pt?.props?.['BreadcrumbLink.href'] ?? 'URL for the breadcrumb link. Omit for the current (last) item.',
            },
            {
              ...
              default: '-',
              description:
                pt?.props?.['BreadcrumbSeparator.children'] ??
                'Custom separator content. Defaults to a chevron SVG icon.',
            },
            {
              ...
              default: '-',
              description:
                pt?.props?.['BreadcrumbEllipsis'] ??
                'Renders a collapsed indicator (...) for hidden intermediate items.',
            },
          ]}
        />
      </DocSection>
