  ...
  return (
    <DocPage>
      <div>
        ...
        <DocDescription>
          {pt?.description ?? 'Font families, sizes, and weights used across the design system.'}
        </DocDescription>
      </div>
      ...
      <DocSection title={pt?.sections?.['font-families']?.title ?? 'Font Families'}>
        <DocText>
          {pt?.sections?.['font-families']?.texts?.[0] ?? (
            <>
              The primary font is set via <InlineCode>--font-primary</InlineCode> and used for all UI text. A secondary
              font <InlineCode>--font-secondary</InlineCode> is available for display or branding.
            </>
          )}
        </DocText>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="p-5 rounded-[var(--radius-lg)] border border-solid border-[var(--border)]">
            ...
            <p className="text-2xl font-medium" style={{ fontFamily: 'var(--font-primary)' }}>
              Aa Bb Cc 123
            </p>
      ...
      <DocSection title={pt?.sections?.['type-scale']?.title ?? 'Type Scale'}>
        <DocText>
          {pt?.sections?.['type-scale']?.texts?.[0] ?? 'A harmonious scale from caption (12px) to display (36px).'}
        </DocText>
        <VStack gap="none">
          {samples.map((s) => (
            <React.Fragment key={s.label}>
              <div className="flex flex-col sm:flex-row sm:items-baseline gap-1 sm:gap-4 py-4">
                <span className="text-[11px] font-semibold uppercase tracking-wider text-[var(--muted-foreground)] w-24 shrink-0">
                  {s.label}
                </span>
