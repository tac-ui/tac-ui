  ...
  return (
    <DocPage>
      <div>
        ...
        <DocDescription>
          {pt?.description ?? 'Tac UI uses Lucide React for icons. Click any icon to copy its import statement.'}
        </DocDescription>
      </div>
      ...
      <DocSection title={pt?.sections?.['usage']?.title ?? 'Usage'}>
        <CodeBlock
          language="tsx"
          code={`// Install\nnpm install @tac-ui/icon\n\n// Import\nimport { Search, Heart, Bell } from '@tac-ui/icon';\n\n// Use with size prop\n<Search size={20} />\n<Heart size={16} strokeWidth={2.5} />`}
        />
      </DocSection>
      ...
      <DocSection title={pt?.sections?.['all-icons']?.title ?? 'All Icons'}>
        <DocText>
          {pt?.sections?.['all-icons']?.texts?.[0] ??
            'Lucide provides 1500+ icons. Visit lucide.dev for the full library and search.'}
        </DocText>
      </DocSection>
    </DocPage>
  );
