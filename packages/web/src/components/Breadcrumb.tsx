...
export const BreadcrumbList = forwardRef<HTMLOListElement, React.HTMLAttributes<HTMLOListElement>>(
  ({ className, ...props }, ref) => (
    <ol
      ref={ref}
      className={cn('flex items-center gap-1.5 flex-wrap text-sm text-[var(--muted-foreground)]', className)}
      {...props}
    />
  ),
);
...
export const BreadcrumbItem = forwardRef<HTMLLIElement, BreadcrumbItemProps>(
  ({ className, current, ...props }, ref) => (
    <li
      ref={ref}
      className={cn('inline-flex items-center gap-1.5', current && 'text-[var(--foreground)] font-medium', className)}
...
export const BreadcrumbLink = forwardRef<HTMLAnchorElement, BreadcrumbLinkProps>(({ className, ...props }, ref) => (
  <a
    ref={ref}
    className={cn('cursor-pointer hover:text-[var(--foreground)] transition-colors', className)}
    {...props}
  />
));
...
export const BreadcrumbEllipsis = forwardRef<HTMLSpanElement, React.HTMLAttributes<HTMLSpanElement>>(
  ({ className, ...props }, ref) => (
    <span ref={ref} className={cn('flex items-center justify-center w-6 h-6', className)} {...props}>
      ...
    </span>
  ),
);
