  ...
  'inline-flex items-center justify-center py-1.5 px-3 text-xs font-medium rounded-[var(--radius-pill)] whitespace-nowrap',
  {
    variants: {
      variant: {
        ...
        glass: 'bg-[var(--interactive-surface-tint)] [backdrop-filter:blur(40px)_saturate(180%)] text-[var(--foreground)] border border-[var(--glass-border)]',
      },
    },
...
export type BadgeVariant = 'default' | 'secondary' | 'destructive' | 'outline' | 'success' | 'error' | 'warning' | 'info' | 'glass';
  ...
  ({ className, variant, interactive, onClick, count, children, ...props }, ref) => {
    ...
    if (isInteractive) {
      return (
        <motion.span
          ref={ref}
          whileHover={{ scale: 1.02, transition: diaSpring.light }}
          whileTap={{ scale: 0.98 }}
          onClick={onClick}
