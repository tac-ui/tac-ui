...
const snackbarVariants = cva(
  'group fixed bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-3 pl-5 pr-4 py-3.5 min-w-[320px] max-w-[560px] w-auto rounded-[var(--radius-xl)] z-[var(--z-toast)] [backdrop-filter:blur(40px)_saturate(180%)] bg-[var(--card)]',
  {
    variants: {
      variant: {
 ...
 */
export interface SnackbarProps extends Omit<React.HTMLAttributes<HTMLDivElement>, MotionConflictingHandlers>, VariantProps<typeof snackbarVariants> {
...
export const Snackbar = forwardRef<HTMLDivElement, SnackbarProps>(
  ({ className, variant = 'default', open, onClose, autoHideDuration = 5000, icon, action, closeLabel = 'Close', onAction, children, ...props }, ref) => {
    ...
    return (
      <motion.div
        ...
        {...props}
      >
        {variant !== 'default' && <span className={cn('w-2 h-2 rounded-full shrink-0', dotColorMap[variant || 'default'])} />}
        {icon && variant === 'default' && <span className="w-[18px] h-[18px] shrink-0 text-[var(--muted-foreground)] [&>svg]:w-[18px] [&>svg]:h-[18px]">{icon}</span>}
        ...
        {action && (
          <button
            ...
            className={cn(
              ...
              variant === 'default' ? 'bg-transparent text-[var(--point)]' : 'bg-[var(--secondary)] text-[var(--foreground)]',
            )}
          >
            {action}
          </button>
