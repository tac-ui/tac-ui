...
export const SegmentController = forwardRef<HTMLDivElement, SegmentControllerProps>(
  (props, ref) => {
    const { className, options, mode = 'single', size = 'md', fullWidth = false, disabled, collapsible = false, ...rest } = props;
      ...
      (e: React.KeyboardEvent<HTMLDivElement>) => {
        if (isMulti) {
          ...
          const btns = Array.from(
            e.currentTarget.querySelectorAll<HTMLButtonElement>('[role="checkbox"]'),
          );
    ...
    const {
      ...
      ...domProps
    } = rest as SegmentControllerBaseProps & { mode?: string; value?: unknown; defaultValue?: unknown; onChange?: unknown; collapsible?: boolean };
    ...
    const renderButton = (option: SegmentOption, isActive: boolean, isDisabled: boolean) => (
      ...
      >
        ...
        {option.icon && (
          <span className="relative z-10 w-4 h-4 [&>svg]:w-4 [&>svg]:h-4">{option.icon}</span>
        )}
    ...
    const container = (
      <div
        ...
        onBlur={collapsible ? (e) => { if (!e.currentTarget.contains(e.relatedTarget as Node)) setIsExpanded(false); } : undefined}
        className={cn(
